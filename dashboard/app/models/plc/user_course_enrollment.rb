    ...
    transaction do
      ...
      learning_modules.each do |learning_module|
        ...
        learning_module.plc_tasks.each do |task|
          Plc::EnrollmentTaskAssignment.find_or_create_by(plc_enrollment_module_assignment: module_assignment, plc_task: task, status: :not_started, type: task.task_assignment_type)
        end
      end
    end
